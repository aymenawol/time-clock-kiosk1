  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Link href="/driver/forms" className="block">
          <Card className="p-4 text-center transition-colors hover:bg-accent">
            <FileText className="mx-auto mb-1 h-6 w-6 text-primary" />
            <div className="text-xs text-muted-foreground">Forms</div>
          </Card>
        </Link>
        <Link href="/driver/safety-meetings" className="block">
          <Card className="p-4 text-center transition-colors hover:bg-accent">
            <ShieldCheck className="mx-auto mb-1 h-6 w-6 text-primary" />
            <div className="text-xs text-muted-foreground">Safety Meetings</div>
          </Card>
        </Link>
        <Link href="/driver/lost-found" className="block">
          <Card className="p-4 text-center transition-colors hover:bg-accent">
            <Search className="mx-auto mb-1 h-6 w-6 text-primary" />
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 bg-muted/60 flex items-center justify-between">
          ...
          <span className="text-warn text-xs">{bothBuses.length} bus{bothBuses.length !== 1 ? 'es' : ''}</span>
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 bg-muted/60 flex items-center justify-between">
          ...
          <span className="text-warn text-xs">{fuelingBuses.length} bus{fuelingBuses.length !== 1 ? 'es' : ''}</span>
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 bg-muted/60 flex items-center justify-between">
          ...
          <span className="text-info text-xs">{washBuses.length} bus{washBuses.length !== 1 ? 'es' : ''}</span>
        </div>
      ...
      <Card className="overflow-hidden">
        ...
        {readyBuses.length === 0 ? (
          <p className="text-muted-foreground text-sm px-4 py-3">No ready buses.</p>
        ) : (
            ...
            {readyBuses.map((b: any) => (
              <div key={b.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {b.fuel_level != null && (
                    <span className={`text-xs font-medium ${b.fuel_level >= 50 ? 'text-ok' : 'text-warn'}`}>
