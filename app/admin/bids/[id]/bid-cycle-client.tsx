  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <a href="/admin/bids" className="text-sm text-gray-500 hover:text-gray-300 mb-1 block">← Bid Cycles</a>
          <h1 className="text-2xl font-bold text-white">{cycle.name}</h1>
          {cycle.description && <p className="text-gray-400 text-sm mt-0.5">{cycle.description}</p>}
          ...
          <div className="flex gap-2">
            ...
            {cycle.status === 'published' && (
              <button onClick={() => statusAction('locked')} disabled={pending}
                className="px-3 py-1.5 bg-yellow-700 hover:bg-yellow-600 text-white text-xs rounded disabled:opacity-50">
                Lock Submissions
              </button>
            )}
            {cycle.status === 'locked' && (
              <>
                ...
                <button onClick={() => statusAction('awarded')} disabled={pending}
                  className="px-3 py-1.5 bg-purple-700 hover:bg-purple-600 text-white text-xs rounded disabled:opacity-50">
                  Mark Awarded
                </button>
              </>
            )}
        ...
        <div className="space-y-3">
          {cycle.status === 'draft' && (
            <button onClick={() => setShowSlotForm(v => !v)}
              className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white text-xs rounded border border-gray-700">
              {showSlotForm ? 'Cancel' : '+ Add Slot'}
            </button>
          )}

          {showSlotForm && (
            <form onSubmit={handleAddSlot} className="bg-gray-900 border border-gray-700 rounded-lg p-4 space-y-3">
              <h3 className="text-sm font-semibold text-white">New Bid Slot</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Bid #</label>
                  <input name="bid_number" type="number" required min={1} className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Route Type</label>
                  <select name="route_type" required className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm">
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Report Time</label>
                  <input name="report_time" type="time" required className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Shift Start</label>
                  <input name="shift_start" type="time" required className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Shift End</label>
                  <input name="shift_end" type="time" required className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Max Drivers</label>
                  <input name="max_drivers" type="number" min={1} defaultValue={1} required className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  {DAY_KEYS.map((d, i) => (
                    <label key={d} className="flex flex-col items-center gap-1 text-xs text-gray-400">
                      {DAY_LABELS[i]}
              ...
              <div>
                ...
                <input name="notes" className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-white text-sm" />
              </div>
          ...
          {slots.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {slots.map(slot => (
                    <tr key={slot.id} className="border-b border-gray-800/50 hover:bg-gray-900/30">
                      <td className="py-2 pr-4 text-white font-mono">{slot.bid_number}</td>
                      <td className="py-2 pr-4 text-gray-300">{ROUTE_TYPE_LABELS[slot.route_type]}</td>
                      <td className="py-2 pr-4 text-gray-300 font-mono">{slot.report_time}</td>
                      <td className="py-2 pr-4 text-gray-300 font-mono">{slot.shift_start} → {slot.shift_end}</td>
                      <td className="py-2 pr-4 text-gray-400 font-mono text-xs">
                        {DAY_KEYS.map((d, i) => (slot as any)[`days_${d}`] ? DAY_LABELS[i] : '·').join(' ')}
                      </td>
                      <td className="py-2 pr-4 text-gray-300">{slot.max_drivers}</td>
                      <td className="py-2 pr-4 text-gray-300">{slot.awards?.length ?? 0}</td>
                      {cycle.status === 'draft' && (
                        <td className="py-2">
              ...
              {submissions.map(sub => (
                <div key={sub.id} className="bg-gray-900 border border-gray-800 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-gray-500 text-xs">Seniority #{sub.employees?.seniority_number ?? '—'}</span>
                  </div>
      ...
      {tab === 'awards' && (
        <div>
          {awards.length === 0 ? (
            <p className="text-gray-500 text-sm">No awards yet. Lock cycle and run award engine.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  {awards.map(aw => (
                    <tr key={aw.id} className="border-b border-gray-800/50">
                      <td className="py-2 pr-4 text-white">{(aw as any).employees?.name ?? aw.employee_id}</td>
                      <td className="py-2 pr-4 text-gray-300">Bid #{(aw as any).shift_bid_slots?.bid_number ?? '?'}</td>
                      <td className="py-2 pr-4 text-gray-400">{aw.preference_rank ?? '—'}</td>
                      <td className="py-2 pr-4 text-gray-400">{aw.award_method}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
