    ...
    <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-4">
          <h2 className="text-white font-semibold">Create Pay Period</h2>
          {error && <p className="text-red-400 text-sm">{error}</p>}
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-gray-400 text-xs block mb-1">Period Start</label>
              <input name="period_start" type="date" required
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div>
              <label className="text-gray-400 text-xs block mb-1">Period End</label>
              <input name="period_end" type="date" required
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div>
              <label className="text-gray-400 text-xs block mb-1">Pay Date</label>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={isPending}
              className="bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-50">
      ...
      <div className="space-y-3">
        {periods.length === 0 && (
          <p className="text-gray-500 text-center py-12">No pay periods created yet.</p>
        )}
        {periods.map(period => (
          <a
            key={period.id}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                {period.exports_count > 0 && (
                  <span className="text-gray-500 text-xs">{period.exports_count} export(s)</span>
                )}
