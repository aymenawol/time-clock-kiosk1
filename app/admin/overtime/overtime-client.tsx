  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      <section className="bg-card border border-border rounded-lg p-4">
        ...
        <form onSubmit={handleBannerSave} className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs ${bannerActive ? 'text-green-400' : 'text-muted-foreground'}`}>{bannerActive ? 'Visible to drivers' : 'Hidden'}</span>
          </div>
          <div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setShowPostForm(v => !v)}
            className="px-3 py-1.5 bg-muted hover:bg-gray-700 text-foreground text-xs border border-border rounded">
            {showPostForm ? 'Cancel' : '+ Post Shift'}
          </button>
        </div>

        {showPostForm && (
          <form onSubmit={handlePostShift} className="bg-card border border-border rounded-lg p-4 mb-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input name="duration_hours" type="number" step="0.5" min="0.5" required className="w-full bg-muted border border-border rounded px-2 py-1.5 text-foreground text-sm" />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Slots Available</label>
                <input name="slots_available" type="number" min="1" defaultValue={1} className="w-full bg-muted border border-border rounded px-2 py-1.5 text-foreground text-sm" />
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Bid Close At</label>
        ...
        {shifts.length === 0 ? (
          ...
          <div className="space-y-2">
            {shifts.map(shift => (
              <div key={shift.id} className="bg-card border border-border rounded-lg p-3 flex items-center justify-between gap-4">
                <div>
      ...
      <section>
        ...
        {showOffDayForm && (
          <form onSubmit={handleSendOffDay} className="bg-card border border-border rounded-lg p-4 mb-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
        ...
        {offDayRequests.length === 0 ? (
          ...
          <div className="space-y-2">
            {offDayRequests.map(req => (
              <div key={req.id} className="bg-card border border-border rounded-lg p-3 flex items-start justify-between gap-3">
                <div>
