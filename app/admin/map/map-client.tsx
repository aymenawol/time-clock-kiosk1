...
export default function MapClient({ initialBuses, apiKey }: Props) {
  ...
  // N9 — chronological trail (oldest→newest) per bus, in state so the polyline re-renders.
  const [trails, setTrails] = useState<Record<string, { lat: number; lng: number }[]>>({})

  // Seed history from initial data
  useEffect(() => {
    const seededTrails: Record<string, { lat: number; lng: number }[]> = {}
    initialBuses.forEach(b => {
      if (b.position) {
        positionHistoryRef.current[b.bus_id] = [b.position]
        seededTrails[b.bus_id] = [{ lat: b.position.latitude, lng: b.position.longitude }]
      }
    })
    setTrails(seededTrails)
  }, [initialBuses])
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Update position history (keep last samples for ETA rolling avg)
          const hist = positionHistoryRef.current[pos.bus_id] ?? []
          positionHistoryRef.current[pos.bus_id] = [pos, ...hist].slice(0, 5)

          // N9 — append to the chronological trail (oldest→newest) for the polyline.
          setTrails(prev => {
            const trail = prev[pos.bus_id] ?? []
            return {
              ...prev,
              [pos.bus_id]: [...trail, { lat: pos.latitude, lng: pos.longitude }].slice(-TRAIL_LENGTH),
            }
          })
        ...
        <div className="flex-1">
          <APIProvider apiKey={apiKey}>
            ...
            >
              ...
              {/* N9 — recent route trail for the selected bus */}
              {selected && (trails[selected.bus_id]?.length ?? 0) >= 2 && (
                <RoutePolyline path={trails[selected.bus_id]} />
              )}
              ...
              {selected && selected.position && (
                ...
                >
                  <div className="text-gray-900 text-sm min-w-48">
                    ...
                    <hr className="my-1" />
                    <p className="font-semibold text-xs uppercase tracking-wide text-muted-foreground mt-1">ETA to terminals</p>
                    {TERMINALS.map(t => {
              ...
              >
                ...
                {bus.position ? (
                  <>
                    ...
                    {(() => {
                      const hist = positionHistoryRef.current[bus.bus_id] ?? []
                      const samples: PositionSample[] = hist.map(p => ({
                        lat: p.latitude, lng: p.longitude, recorded_at: p.recorded_at, speed: p.speed,
                      }))
